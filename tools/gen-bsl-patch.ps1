# Generates patches/bsl.irlpatch by splicing the IRLite bodies VERBATIM out of
...
Emit '# IRLite — point + spot lights for BSL Shaders v10, ported from the'
Emit '# Complementary inject: diffuse + specular + real-time shadows + volumetrics'
Emit '# + toon + rim outline, plus an in-shader settings screen (Iris: Shader Pack'
Emit '# Settings -> Lighting -> IRLite Lights).'
Emit '#'
Emit '# BSL is FORWARD-shaded: diffuse+specular hook GetLighting() in'
Emit '# lib/lighting/forwardLighting.glsl (covers all 12 includer programs; the'
Emit '# entity flag is compile-time via per-program defines). The volumetric march'
Emit '# runs in an ADDED reduced-resolution pass (deferred2 -> colortex10, sized by'
Emit '# the IRLITE_VL_RESOLUTION option) and program/composite.glsl upsamples it'
Emit '# and hosts the rim outline — composite, NOT composite1, which the pack'
Emit '# program-toggles off whenever LIGHT_SHAFT is disabled. No wrapper edits:'
Emit '# the inject carries its own in-file #extension lines (Iris transpiles the'
Emit '# #version 120 pack to 330 core). Specular is IRLite-only on entities/hand/'
Emit '# block entities — vanilla BSL blocks carry no smoothness data and stay'
Emit '# matte. Requires Iris (SSBO) — the patched pack will not load on OptiFine.'
