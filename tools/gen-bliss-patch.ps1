...
Emit '# IRLite - point + spot lights for Bliss (by X0nk, Chocapic13 lineage), ported'
Emit '# from the Solas inject: diffuse + specular + real-time shadows + volumetrics'
Emit '# + toon + rim outline, plus an in-shader settings screen (Iris: Shader Pack'
Emit '# Settings -> IRLite Lights on the main page; English AND Russian labels).'
Emit '#'
Emit '# Bliss is DEFERRED with a forward translucent path, so the surface hook is'
Emit '# DUAL: dimensions/composite1.fsh (deferred opaque lighting - entity/hand'
Emit '# flags come from the pack''s native masks; feetPlayerPos there is already'
Emit '# view-bobbing-corrected, never add MVI[3] again) AND'
Emit '# dimensions/all_translucent.fsh (water + translucent entities = BBS replay'
Emit '# models when TRANSLUCENT_ENTITIES is on; flags via the wrapper defines).'
Emit '# The volumetric march PIGGYBACKS the pack''s own reduced-resolution VL pass'
Emit '# (dimensions/composite2.fsh, added once after the main write and re-added'
Emit '# after the pack''s underwater overwrite) - no extra passes, buffers or'
Emit '# wrappers; the pack''s composite3 bilateral upsample consumes it for free.'
Emit '# The rim outline inks in dimensions/composite3.fsh (the post-translucent'
Emit '# merge pass) before the fog merge. The pack is #version 120 and the inject'
Emit '# carries its own in-file #extension lines; iris.features gets SSBO appended'
