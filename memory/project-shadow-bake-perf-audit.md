...
2026-07-18: GPU-профайлер намерил bake = 3.4-4.0ms GPU СТАБИЛЬНО каждый кадр (overlay-цепочка copy+dyn-draw+Pyramid/EVSM whole-block z=6 — ВНЕ таксономии этого аудита) поверх известных C10-спайков ~320ms. Bake-трек стартует по [[plan-shadow-bake-track]] (Ф0 = профайлер-разбивка; рекон-якоря актуального post-atlas-merge кода там же). Этот док остаётся каноном таксономии и вердиктов (рефьюты/удаления НЕ переоткрывать).

Связано: [[project-port-1211]], [[project-irlite-base-ported]], [[plan-irl-core-shadow-extraction]], [[addon-shadows]], [[plan-shadow-bake-track]].
