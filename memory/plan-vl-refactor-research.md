...
СТАТУС 2026-07-17: исследование ЗАВЕРШЕНО. Деливерабл = отчёт ниже (полный текст синтеза). Реализация НЕ начата, тираж НЕ обсуждался. Ветка на момент сессии: optimization/octahedral-point-shadows (dirty, atlas-merge не закоммичен — ортогонально, юзер подтвердил «всё норм»).
