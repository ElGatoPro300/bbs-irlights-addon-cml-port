...
Ограничение по редактору (2026-07-04, коммит 3913622, компиляция PASS, ин-гейм pending): гайды/хват-зоны спота в film editor показываются ТОЛЬКО в replay-редакторе, НЕ в камера-редакторе и НЕ в actions-таймлайне (в них тот же реплей остаётся выбранным, BBS продолжает его пикать). Единый гейт SpotGuideDrag.isReplayEditorActive() = film.replayEditor.isVisible() && !film.replayEditor.isActionsMode() (публичный BBS API, BBS не патчится; isVisible — родной сигнал BBS «активен ли replay-редактор», UIFilmController:705). Точки применения: isFilmSelected() -> false вне replay-редактора (мгновенное скрытие) + AbstractLightFormRenderer.render3D ветка filmPick гейтит markFilmSelected И renderStencilHandles (хэндлы не регаются в стенсил -> ни драга, ни перехвата ховер-лейблов BBS). Превью редактора форм (Ф1, context.modelRenderer) не затронуто.
