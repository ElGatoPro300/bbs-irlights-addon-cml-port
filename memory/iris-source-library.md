...
Две копии, обе полные Gradle-проекты (build.gradle.kts, settings.gradle.kts, README.md, docs/, gradle/, DHApi.jar). ПУТИ ОБНОВЛЕНЫ 2026-07-12: дерево IRLEngine под BBS/ больше не существует (recon Phase 3 это подтвердил), актуальное место — под bbs-dof-addon:
...
- Fallback (Iris 1.7.2 для MC 1.20.4 — близко, не точно): C:\Users\Qualet\Documents\Project\Minecraft\BBS\bbs-dof-addon\Source-libary\Iris-1.7.2-1.20.4 (если отсутствует — сверять символы против рантайм-jar в loom-cache аддона: .gradle/loom-cache/remapped_mods/remapped/maven/modrinth/iris-a73a62ac/1.7.2+1.20.4/, приём javap подтверждён 2026-07-12 мижином кластера)
