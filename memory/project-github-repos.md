...
bbs-dof-addon = 4-й репо, НАМЕРЕННО LOCAL-ONLY (решение юзера 2026-07-04): remote НЕ настроен, GitHub-репо quaIett/bbs-dof-addon НЕ существует (gh 404). Причина = бандлит combo-патчи под приватные/платные шейдеры (IterationRP Tahnass и др.) — тот же контент, что вычищался из трилогии для публикации (см. блок «IterationRP убран»). НЕ создавать remote / НЕ пушить без явной прямой команды. При «пушни все репо» — bbs-dof-addon ПРОПУСКАТЬ (коммитить локально можно, коммит 736fa7d пример). Пуш трилогии 2026-07-04: irl-core main->6668f22, bbs-irlights-addon master->283256b, irl-editor main->57a7dfd (все fast-forward, origin у каждого).
